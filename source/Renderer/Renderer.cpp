...
constexpr mr::VkDeviceCapabilities operator|(mr::VkDeviceCapabilities a,
                                             mr::VkDeviceCapabilities b) {
  return static_cast<mr::VkDeviceCapabilities>(static_cast<uint64_t>(a) | static_cast<uint64_t>(b));
}
...
constexpr bool hasFlag(mr::VkDeviceCapabilities caps, mr::VkDeviceCapabilities flag) {
  return (static_cast<uint64_t>(caps) & static_cast<uint64_t>(flag)) == static_cast<uint64_t>(flag);
}

void mr::VkRenderer::init() {
  try {
...
void mr::VkRenderer::initValidationLayers() {
  m_validationLayers = {"VK_LAYER_KHRONOS_validation"};
}

void mr::VkRenderer::initExtensions() {
...
void mr::VkRenderer::createInstance() {
...
void mr::VkRenderer::createSurface() {
...
void mr::VkRenderer::pickPhysicalDevice() {
...
void mr::VkRenderer::createDevice() {
  // pQueueCreateInfos
  float priority = 1.0f;
...
void mr::VkRenderer::createSwapchain() {
  // Capabilities
  ...
  for (uint32_t i = 0; i < m_swapchainImages.size(); ++i) {
    ...
    viewInfo.format = m_swapchainSurfaceFormat.format;
    ...
    if (vkCreateImageView(m_logicalDevice, &viewInfo, nullptr, &m_swapchainImageViews[i])
        != VK_SUCCESS)
      throw std::runtime_error("[ERROR](VkRenderer){Failed to create image view}");
  }
}

void mr::VkRenderer::createPipeline() {
  // Render pass
  VkAttachmentDescription colorAttachmentDesc{};
  colorAttachmentDesc.flags = 0;
  colorAttachmentDesc.format = m_swapchainSurfaceFormat.format;
  ...
  if (vkCreateRenderPass(m_logicalDevice, &renderpassCreateInfo, nullptr, &m_renderPass))
  ...
  shaderStageCreateInfo[0].module = 0;
  shaderStageCreateInfo[0].pName = "vertMain";
  ...
  graphicsPipelineInfo.flags = VK_PIPELINE_CREATE_DISABLE_OPTIMIZATION_BIT;
}

VKAPI_ATTR VkBool32 VKAPI_CALL
mr::VkRenderer::debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
...
std::optional<uint32_t> mr::VkRenderer::getFamilyQueue(VkPhysicalDevice physicalDevice,
...
void mr::VkRenderer::checkValidationLayerSupport(const std::vector<const char*>& required) {
...
void mr::VkRenderer::checkInstanceExtensions(const std::vector<const char*>& requiredExt) {
  uint32_t extensionCount = 0;
...
std::optional<uint32_t> mr::VkRenderer::findPresentQueueSupport(VkPhysicalDevice physicalDevice,
