...
uint64_t mr::EntityRegistry::createEntity(const std::string& name, EntityTag tag) {
  Entity e{this};
...
void mr::EntityRegistry::deleteEntity(const ::std::string& name) {
...
mr::Entity* mr::EntityRegistry::getEntity(const std::string& name) {
  auto it = m_nameToID.find(name);
  if (it == m_nameToID.end())
    return nullptr;
...
mr::Entity* mr::EntityRegistry::getEntity(uint64_t id) {
