...
public:
  ...
  ResourceHandle<T> load(const std::string& resourceId, const std::string& path, Args&&... args) {
    ...
    GraphicsAPI& api = getSettings()->m_graphicsApi;

    // Create resource
    std::shared_ptr<Resource> resource;
    if constexpr (std::is_same_v<T, Texture>) {
      if (api == GraphicsAPI::Vulkan)
        resource = std::make_shared<VkTexture>(resourceId, path, std::forward<Args>(args)...);
    } else if constexpr (std::is_same_v<T, Mesh>) {
      if (api == GraphicsAPI::Vulkan)
        resource = std::make_shared<VkMesh>(resourceId, path, std::forward<Args>(args)...);
    } else if constexpr (std::is_same_v<T, Shader>) {
      if (api == GraphicsAPI::Vulkan)
        resource = std::make_shared<VkShader>(resourceId, path, std::forward<Args>(args)...);
    }
...
class AsyncResourceManager {
  ...
  std::unique_ptr<SlangCompiler> m_slangCompiler = nullptr;
...
public:
  ...
  template <typename T, typename... Args>
  void loadResource(const std::string& resourceId, const std::string& path, Args&&... args) {
    auto argsTuple = std::make_tuple(std::forward<Args>(args)...);
    {
      std::lock_guard<std::mutex> lock(m_queueMutex);
      m_taskQueue.push([this, resourceId, path, argsTuple]() {
        std::apply(
          [&](auto&&... unpackedArgs) {
            ResourceHandle<T> handle = m_resourceManager.load<T>(resourceId, path, unpackedArgs...);
          },
          argsTuple);
      });
    }
...
template <typename T>
T* mr::ResourceHandle<T>::get() const {
  if (!m_manager)
    return nullptr;
...
template <typename T>
bool mr::ResourceHandle<T>::isValid() const {
  return m_manager && m_manager->hasResource<T>(m_id);
}

template <typename T>
T* mr::ResourceHandle<T>::operator->() const {
  return get();
}
template <typename T>
T& mr::ResourceHandle<T>::operator*() const {
...
template <typename T>
mr::ResourceHandle<T>::operator bool() const {
  return isValid();
}
